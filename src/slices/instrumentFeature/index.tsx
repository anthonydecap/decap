...
const InstrumentFeature: FC<SliceComponentProps<any>> = ({ slice }) => {
  ...
  return (
    <section className={`${INSTRUMENT_SECTION_PY_LG} text-white`} style={{ backgroundColor: bg }}>
      <Container>
        <div className="grid min-h-[420px] grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <FadeIn>
            {p.title && (
            ...
            {p.body && (
              <div className="mt-6 max-w-xl text-lg leading-relaxed text-neutral-400">
                <PrismicRichText field={p.body} components={instrumentRichText} />
              </div>
            )}
          </FadeIn>
          <FadeIn>
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900 lg:rounded-3xl">
              {useMux && muxUrl ? (
                <MuxVideoLayer src={muxUrl} className="absolute inset-0 h-full w-full" />
              ) : p.image?.url ? (
                <PrismicNextImage field={p.image} className="h-full w-full object-cover" alt="" />
              ) : (
